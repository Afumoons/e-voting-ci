			...
			<div class="col-md-8">
					...
					<select class="custom-select" id="jurusan" name="jurusan">
						<?php foreach ($jurusan as $item) : ?>
							...
							<?php if (session()->id_level == 1) : ?>
								<option value="<?= $item['id_jurusan'] ?>" <?= $kandidat['id_jurusan'] == $item['id_jurusan'] ? 'selected=""' : ''; ?>><?= $item['jurusan'] ?></option>
							<?php else : ?>
								<!-- Cek sesuai jurusan -->
				...
				<div class="form-group">
					...
					<select class="custom-select" id="ormawa" name="ormawa">
						<option value="bem" <?= ($kandidat['ormawa'] == 'bem') ? 'selected=""' : '' ?>>Badan Eksekutif</option>
						<option value="blm" <?= ($kandidat['ormawa'] == 'blm') ? 'selected=""' : '' ?>>Badan Legislatif</option>
					</select>
				</div>
